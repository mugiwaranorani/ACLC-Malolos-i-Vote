...
class User{
	public function check_login($usn, $pswd){
		...
		$query = "SELECT voterID, fullName, usn FROM clg_voter WHERE usn = '$usn' AND vpass = '$pswd'";
	...
	public function get_voterid($voterID){
		include "dbconn.php";
		$query = "SELECT voterID FROM clg_voter WHERE voterID = $voterID";
		
		$result = $conn->query($query);
		
		$user_data = $result->fetch_array(MYSQLI_ASSOC);
		echo $user_data['voterID'];
		mysqli_close($conn);
	}
  ...
  public function get_usn($voterID){
		include "dbconn.php";
		$query = "SELECT usn FROM clg_voter WHERE voterID = $voterID";
...
if (isset($_POST['submit'])) { 
	    ...
	    if ($login) {
     	...
     	$usn = $_SESSION['usn'];
			...
			$sql = "INSERT INTO `clg_userlog` (`voterID`, `logName`, `userIP`, logUSN)	VALUES ('$voterID', '$fullName', '$uip', '$usn')";
	        ...
	        // login Failed
	        header("location:collegeLogin-Invalid.php");
	    }
}
?>