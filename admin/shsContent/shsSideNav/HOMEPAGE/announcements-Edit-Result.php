          ...
          <div class="tab-pane fade show active p-3">
            ...
            <div class="container-fluid overflow-auto p-1" style="height: 500px;">
              ...
              <div class="container home-content p-3">        
                ...
                <?php
                  if (isset($_POST['editannounceBtn'])) {
                      include ('../../../../dbconn.php');
                      ...
                      if (!empty($image)) {
                          ...
                          if (move_uploaded_file($image_tmp, $target_path)) {
                              ...
                              $contentHeader = $_POST['announcementcontentheader'];
                              $content = $_POST['announcementcontent'];
                              $contentHeader = str_replace('\'', '’', $contentHeader);
                              $content = str_replace('\'', '’', $content);
                              $id = $_REQUEST['id'];

                              $sql = "UPDATE `shs_home` SET `contentHeader`='$contentHeader', `content`='$content', `homeImages`='$image' WHERE homeID = $id";
                              $result = $conn->query($sql);

                              if ($result === TRUE) {
                                  // Query executed successfully
                                  ?>
                                  <i><h2 class="text-success">Announcement content has been updated successfully!</i></h2>
                                  <?php
                              } else {
                          ...
                          $sql = "UPDATE `shs_home` SET `contentHeader`='$contentHeader', `content`='$content' WHERE homeID = $id";
                          $result = $conn->query($sql);
