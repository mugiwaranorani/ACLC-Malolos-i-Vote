          ...
          <div class="tab-pane fade show active p-3">
            ...
            <div class="container-fluid overflow-auto rounded p-2" style="height: 550px;">
              ...
              <?php			
                include ('../../../../dbconn.php');
                $id = $_REQUEST['id'];
                $sql = "SELECT * FROM shs_title WHERE titlePage = $id";
                $result = $conn->query($sql);
                                      
                if ($result->num_rows > 0) {
                  while($row = $result->fetch_array()){
              ?>
              <div class="container line-shadow p-3 rounded">
                
                <form action="title-edit-result.php" method="post">

                  <h3><i class='fas fa-edit' style='font-size:30px'></i> EDIT <?php echo strtoupper($row['titlePage'])?> TITLE</h3>
                  <hr>
                  <label class="text-danger"><h5>Note: The numbers beside the labels, indicates the maximum letters/characters that you can input in each form.</h5></label>
                    ...
                    <div class="form-floating mb-3">
                      <input type="text" class="form-control" id="Title" name="Title" placeholder="title here" value="<?php echo $row['title']?>" required>
                      <label for="floatingInput"><?php echo $row['titlePage']?> Title<span class="text-danger"> (100)</span>:</label>
                    </div>
                        
                    <input class='btn btn-primary' type="hidden" name="id" value="<?php echo $_REQUEST['id'];?>">
                    <input type="submit"  name="editTitle" class="btn btn-success px-4" value="Save">
                    <a class="btn btn-danger px-3" href="titleControl.php">Cancel</a>
