          ...
          <div class="tab-pane fade show active p-3">
            ...
            <div class="container-fluid overflow-auto" style="height: 560px;">
              ...
              <div class="container line-shadow p-3">
                ...
                <h3><i class='fas fa-user-times' style='font-size:30px'></i> DELETE VOTER ACCOUNT</h3>
                <hr>
                  
                  <?php			
                    include ('../../../../dbconn.php');
                    $id = $_REQUEST['id'];
                    $sql = "SELECT * FROM clg_voter WHERE voterID=$id";
                    $result = $conn->query($sql);
                              
                    if ($result->num_rows > 0) {
                      while($row = $result->fetch_array()) {
                  ?>
                    <span class='text-danger fs-5'>Are you sure you want to delete <span class="text-secondary fs-4">"<?php echo $row['fullName']?>"</span>'s Account?</span>
                    <?php	
                      }		
                      $conn->close();
                      }
                  ?> 
                    <form action="voter-delete-result.php" method='post'>
                      <input class='btn btn-primary' type="hidden" name="id" value="<?php echo $_REQUEST['id'];?>">
                      <button class="btn btn-success px-4 m-1 mt-2" name="deleteAccInfoBtn">Yes</button>
                      <a class='btn btn-danger px-4 m-1 mt-2' href='votersControl.php'>No</a>
                    </form>
