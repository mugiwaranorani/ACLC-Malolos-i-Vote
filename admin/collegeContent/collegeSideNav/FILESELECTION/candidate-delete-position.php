          ...
          <div class="tab-pane fade show active p-3">
            ...
            <br>
              <div class="container line-shadow p-3">
                ...
                <h3><i class='fas fa-file' style='font-size:30px'></i> DELETE POSITION (SELECTION)</h3>
                <hr>
                    <?php			
                      include ('../../../../dbconn.php');
                      $id = $_REQUEST['id'];
                      $sql = "SELECT * FROM clg_positionselection WHERE positionID=$id";
                      $result = $conn->query($sql);
                              
                      if ($result->num_rows > 0) {
                        while($row = $result->fetch_array()) {
                    ?>
                    <span class='text-danger fs-5'>Are you sure you want to delete <span class="text-secondary fs-4 mt-2">"<?php echo $row['position']?>"</span> from position selection?</span>
                    <?php	
                      }		
                      $conn->close();
                      }
                    ?> 
                    <form action="candidate-delete-position-result.php" method='post'>
                      <input class='btn btn-primary' type="hidden" name="id" value="<?php echo $_REQUEST['id'];?>">
                      <button class="btn btn-success px-4 m-1" name="deletePosInfoBtn">Yes</button>
