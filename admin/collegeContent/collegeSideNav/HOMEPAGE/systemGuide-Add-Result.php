                  ...
                  if (isset($_POST['addguideBtn'])) {
                      ...
                      if (!empty($image)) {
                          ...
                          if (move_uploaded_file($image_tmp, $target_path)) {
                              $hcontentID = $_POST['hiddenguideID'];
                              $deviceType = $_POST['deviceType'];
                              $contentHeader = $_POST['guidecontentheader'];
                              $content = $_POST['guidecontent'];
                              $contentHeader = str_replace('\'', '’', $contentHeader);
                              $content = str_replace('\'', '’', $content);

                              $sql = "INSERT INTO clg_home (homecontentID, device, contentHeader, content, homeImages)
                                      VALUES ('$hcontentID', '$deviceType', '$contentHeader', '$content', '$image')";

                              if ($conn->query($sql) === TRUE) {
                                  ?>
                                  <i><h2 class="text-success">New System Guide content has been added successfully!</i></h2>
                                  <?php
                              } else {
                          ...
                          $sql = "INSERT INTO clg_home (homecontentID, device, contentHeader, content)
                                  VALUES ('$hcontentID', '$deviceType', '$contentHeader', '$content')";
