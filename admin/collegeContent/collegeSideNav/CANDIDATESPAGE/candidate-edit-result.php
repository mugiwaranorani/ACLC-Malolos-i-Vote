            ...
            <div class="container-fluid overflow-auto" style="height: 560px;">
              ...
              <div class="container line-shadow p-3">
                ...
                <?php
                  if (isset($_POST['editCandidateBtn'])) {
                      ...
                      if (!empty($image)) {
                          ...
                          if (move_uploaded_file($image_tmp, $target_path)) {
                              ...
                              $id = $_REQUEST['id'];
                              $sql = "UPDATE `clg_candidates` SET `picture`='$image', `name`='$fname', `position`='$position', `partyList`='$partyList', `yearLvl`='$yearLevel', `course`='$course', `platform`='$platform' WHERE candidateID = $id";
                              $result = $conn->query($sql);

                              if ($conn->query($sql) === TRUE) {
                                  ?>
                                  <br><i><h2 class="text-success">Candidate has been updated successfully!</i></h2>
                                  <?php
                              } else {
                              ...
                              echo "<a class='btn btn-primary px-4' href='candidatesControl.php'>BACK</a>";
                          }
                      } else {
                          $fname = $_POST['fname'];
                          $position = $_POST['pos'];
                          ...
                          $id = $_REQUEST['id'];
                          $sql = "UPDATE `clg_candidates` SET `name`='$fname', `position`='$position', `partyList`='$partyList', `yearLvl`='$yearLevel', `course`='$course', `platform`='$platform' WHERE candidateID = $id";
                          $result = $conn->query($sql);

                          if ($conn->query($sql) === TRUE) {
                              ?>
                              <i><h2 class="text-success">Candidate has been updated successfully!</i></h2>
                              <?php
                          } else {
                              echo "Error: " . $sql . "<br>" . $conn->error;
                          }
                  ...
                  <a class="btn btn-primary px-4 mt-2" href="candidatesControl.php">BACK</a>
